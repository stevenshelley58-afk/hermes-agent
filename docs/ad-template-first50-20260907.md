...
A `ready_for_review` run counts as passed only when its bounded monitor output includes a template summary; geometry, typography, colour/effects, image crop and details scores at or above 9.8 from the comparator and both distinct final reviewers; one aggregate `no_obvious_errors` pass; a quarantined import plus passing smoke test; and `reusable_validation` evidence for all four scenarios (`counts.total == counts.passed == 4`). Exact font-family matching is excluded from scoring, while text sizing, spacing, alignment, hierarchy and legibility remain part of typography. The driver never calls retry, request-changes, approve, publish, or cancel APIs. A nonzero exit means the batch is incomplete or failed and should be inspected from its durable state/ledger before any operator action.
